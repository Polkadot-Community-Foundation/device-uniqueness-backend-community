...
use passes::Passes;
use people::People;
pub use settlement::SettleConfig;
...
pub struct WriterConfig {
    ...
    /// PCF fork: dotNS pending-claim settlement.
...
pub async fn run(config: WriterConfig) -> Result<(), WriterError> {
    ...
    let settlement = Settlement::new(config.settle.clone());
...
impl Writer {
    ...
    async fn settle_pass(&mut self, guard: &Guard) -> Result<(), WriterError> {
        let Writer {
            pool,
            signer,
            signer_account,
        ...
        settlement.tick(&cx, dotns).await
    }
